...
def binary_validation_step(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, criterion: torch.nn.Module, device: torch.device, ths: float = 0.5) -> float:
    """
    ...
    predictions = (predictions >= ths).astype(int)
    ...
    f1_score = metrics.f1_score(ground_truth, predictions, zero_division=0)
    precision = metrics.precision_score(ground_truth, predictions, zero_division=0)
    recall = metrics.recall_score(ground_truth, predictions, zero_division=0)
...
def multiclass_validation_step(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, criterion: torch.nn.Module, device: torch.device) -> float:
    """
    Performs a validation step on the model using the provided dataloader.
    ...
    # Calculate metrics
    accuracy = metrics.accuracy_score(ground_truth, predictions)
    f1_score = metrics.f1_score(ground_truth, predictions, average='weighted', zero_division=0)
    precision = metrics.precision_score(ground_truth, predictions, average='weighted', zero_division=0)
    recall = metrics.recall_score(ground_truth, predictions, average='weighted', zero_division=0)
