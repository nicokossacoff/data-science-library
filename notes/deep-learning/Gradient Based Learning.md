...
- However, due to the non-linearity introduced by the activation functions, the loss functions used for training neural networks are typically **non-convex**[^1]. As a result, neural networks are usually trained using iterative, gradient-based optimization algorithms that aim to reduce the loss to a sufficiently low value.
